...
# This script can be executed from the jumphost to create files on cluster nodes

set -e

echo "Creating required paths and files for CKS exam..."

# Determine target host (default to k8s-api-server if not provided)
TARGET_HOST=${1:-k8s-api-server}
SSH_OPTS="-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null"

echo "Target host: $TARGET_HOST"

# Use SSH to create files on the target host
ssh $SSH_OPTS candidate@$TARGET_HOST bash <<'REMOTE_SCRIPT'
set -e
...
echo "Files created on $TARGET_HOST"
